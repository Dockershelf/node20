...
Debian packaging for Node.js 20: monolithic `nodejs` replacement packages used by the [Dockershelf node-pipeline](../node-pipeline).
...
```bash
make materialize NODE=20 DIST=trixie
make build NODE=20
```
...
| Path | Purpose |
|------|---------|
| `node/` | Upstream Node.js git submodule (`v20.x` branch) |
